...
typedef struct __mavlink_attitude_t {
 uint32_t time_boot_ms; /*< [ms] Timestamp (time since system boot).*/      //32位
 float roll; /*< [rad] Roll angle (-pi..+pi)*/                              //32位
 float pitch; /*< [rad] Pitch angle (-pi..+pi)*/
 float yaw; /*< [rad] Yaw angle (-pi..+pi)*/
 float rollspeed; /*< [rad/s] Roll angular speed*/
 float pitchspeed; /*< [rad/s] Pitch angular speed*/
 float yawspeed; /*< [rad/s] Yaw angular speed*/
} mavlink_attitude_t;           //224位
...
/**
 * @brief Pack a attitude message                               //包装 姿态 信息       status(以MAVLINK_COMM_0作为默认通道)
 * @param system_id ID of this system                               //系统ID
 * @param component_id ID of this component (e.g. 200 for IMU)      //组件ID
 * @param msg The MAVLink message to compress the data into         //要将数据压缩到的MAVLink消息
 *
 * @param time_boot_ms [ms] Timestamp (time since system boot).     //时间戳(系统启动后的时间)
 * @param roll [rad] Roll angle (-pi..+pi)
 * @param pitch [rad] Pitch angle (-pi..+pi)
 * @param yaw [rad] Yaw angle (-pi..+pi)
 * @param rollspeed [rad/s] Roll angular speed                      //滚动角速度
 * @param pitchspeed [rad/s] Pitch angular speed                    //俯仰角速度
 * @param yawspeed [rad/s] Yaw angular speed                        //偏航角速度
 * @return length of the message in bytes (excluding serial stream start sign)  //以字节表示的消息长度(不包括串行流开始符号)
 */
                               ...
                               uint32_t time_boot_ms, float roll, float pitch, float yaw, float rollspeed, float pitchspeed, float yawspeed)
{
    ...
    _mav_put_float(buf, 4, roll);                               //大段模式转换为小端模式
    ...
    _mav_put_float(buf, 24, yawspeed);
        ...
        memcpy(_MAV_PAYLOAD_NON_CONST(msg), buf, MAVLINK_MSG_ID_ATTITUDE_LEN);      //把数据存到 payload64[]数据段
#else
...
/**
 * @brief Pack a attitude message                               //包装 姿态 信息      自己定义 mavlink_status_t   不使用通道
...
static inline uint16_t mavlink_msg_attitude_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               uint32_t time_boot_ms, float roll, float pitch, float yaw, float rollspeed, float pitchspeed, float yawspeed)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS     //0 || 0
    char buf[MAVLINK_MSG_ID_ATTITUDE_LEN];
    _mav_put_uint32_t(buf, 0, time_boot_ms);
...
/**
 * @brief Pack a attitude message on a channel                      //包装 姿态 信息  status(自己设置通道 几)
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
...
/**
 * @brief Encode a attitude struct                                  //调用包装 姿态 信息       status(以MAVLINK_COMM_0作为默认通道)
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param msg The MAVLink message to compress the data into
...
{
    //包装 姿态 信息      自己定义 mavlink_status_t   不使用通道
...
/**
 * @brief Decode a attitude message into a struct                   //将态度信息解码成结构
 *
